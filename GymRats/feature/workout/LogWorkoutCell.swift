...
class LogWorkoutCell: UITableViewCell {
  ...
  @IBOutlet weak var healthAppView: UIView!
  ...
  let shortPressHealth: UILongPressGestureRecognizer = {
    let press = UILongPressGestureRecognizer()
    press.minimumPressDuration = 0.05
    press.cancelsTouchesInView = false
  ...
  override func awakeFromNib() {
    ...
    shortPressHealth.addTarget(self, action: #selector(handleHealth(shortPress:)))
    shortPressHealth.delegate = self

    chooseFromLibraryView.layer.cornerRadius = 4
    takePictureView.layer.cornerRadius = 4
    healthAppView.layer.cornerRadius = 4
    
    chooseFromLibraryView.backgroundColor = .foreground
    takePictureView.backgroundColor = .foreground
    healthAppView.backgroundColor = .foreground
